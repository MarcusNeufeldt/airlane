...
// Non-destructive process modification function
function computeFinalProcessState(currentProcess, newProcess) {
  console.log('🔄 Computing final process state non-destructively');
  console.log('📊 Current process elements:', currentProcess?.elements?.length || 0);
  console.log('📊 New process elements:', newProcess?.elements?.length || 0);
  
  if (!currentProcess || !currentProcess.elements) {
    console.log('📋 No current process - returning new process as-is');
    return newProcess;
  }
  
  if (!newProcess || !newProcess.elements) {
    console.log('📋 No new process - returning current process as-is');
    return currentProcess;
  }
  
  // Create lookup maps for efficient comparison
  const currentElements = new Map(currentProcess.elements.map(e => [e.id, e]));
  const newElements = new Map(newProcess.elements.map(e => [e.id, e]));
  
  const finalElements = [];
  
  // Process all elements from new process
  newProcess.elements.forEach(newElement => {
    const currentElement = currentElements.get(newElement.id);
    
    if (currentElement) {
      // Element exists - merge properties while preserving existing element properties
      console.log(`🔧 Merging element: ${newElement.id}`);
      finalElements.push({
        ...currentElement, // Keep existing properties like position, etc.
        ...newElement, // Update with new element properties
      });
    } else {
      // New element
  ...
  // Handle flows - preserve existing ones that are still valid
  const currentFlowKeys = new Set(
    (currentProcess.flows || []).map(flow => 
      `${flow.source}->${flow.target}`
    )
  );
  
  const newFlowKeys = new Set(
    (newProcess.flows || []).map(flow => 
      `${flow.source}->${flow.target}`
    )
  );
  ...
  // Preserve existing flows that are still present in new process
  (currentProcess.flows || []).forEach(flow => {
    const key = `${flow.source}->${flow.target}`;
    if (newFlowKeys.has(key)) {
      console.log(`↻ Preserving flow: ${key}`);
      finalFlows.push(flow);
    } else {
      console.log(`- Removing flow: ${key}`);
    }
  });
  
  // Add new flows
  (newProcess.flows || []).forEach(flow => {
    const key = `${flow.source}->${flow.target}`;
    if (!currentFlowKeys.has(key)) {
      console.log(`+ Adding new flow: ${key}`);
      finalFlows.push(flow);
    }
  });
  ...
  console.log('✅ Final process computed:', {
    elements: finalProcess.elements.length,
    flows: finalProcess.flows.length
  });
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    // Route: POST /generate-process
    if (method === 'POST' && url.includes('/generate-process')) {
      try {
        const { prompt, existingProcess } = body;
        console.log('🎯 Generating process for:', prompt);
        console.log('📊 Has existing process:', !!existingProcess);
        
        const aiService = new AIService();
        const process = await aiService.generateProcess(prompt, existingProcess);
        console.log('✅ Process generated successfully');
        
        return res.json({ process });
      } catch (error) {
        console.error('❌ Process generation failed:', error.message);
        return res.status(500).json({ 
    ...
    // Legacy Route: POST /generate-schema (for backward compatibility)
    ...
    // Route: POST /analyze-process
    if (method === 'POST' && url.includes('/analyze-process')) {
      try {
        const { process } = body;
        if (!process || !process.elements || process.elements.length === 0) {
          return res.status(400).json({ error: 'Process is required and must not be empty.' });
        }

        console.log('🔬 Analyzing process...');
        const aiService = new AIService();
        const analysis = await aiService.analyzeProcess(process);
    ...
    if (method === 'POST' && url.includes('/chat')) {
      try {
        const { message, currentProcess, currentSchema, conversationHistory } = body;
        console.log('💬 Processing chat message:', message);
        console.log('📊 Current process exists:', !!currentProcess);
        console.log('📊 Current schema exists (legacy):', !!currentSchema);
        ...
        if (response.type === 'message') {
          ...
          return res.json({ content: response.content, process: response.process });
        } else if (response.type === 'tool_call') {
          console.log('🔧 Processing tool call:', response.tool_call.function.name);
          const toolCall = response.tool_call;
          
          // Process-focused tool calls
          if (toolCall.function.name === 'analyze_current_process') {
            console.log('🔍 Executing process analysis');
            const analysis = await aiService.analyzeProcess(currentProcess);
            return res.json({ content: analysis });
          } else if (toolCall.function.name === 'generate_business_process') {
            console.log('🏗️ Executing process generation');
            const args = JSON.parse(toolCall.function.arguments);
            const process = await aiService.generateProcess(args.description);
            return res.json({ process, content: 'I\'ve generated a new business process for you.' });
          } else if (toolCall.function.name === 'modify_existing_process') {
            console.log('🔄 Executing process modification (non-destructive)');
            const args = JSON.parse(toolCall.function.arguments);
            
            // Generate the AI's proposed process
            const aiGeneratedProcess = await aiService.generateProcess(args.description, currentProcess);
            
            // Use non-destructive merging to preserve existing element IDs and positions
            const finalProcess = computeFinalProcessState(currentProcess, aiGeneratedProcess);
            
            return res.json({ process: finalProcess, content: 'I\'ve modified your business process.' });
          }
    ...
    if (method === 'POST' && url.includes('/diagram-chat')) {
      ...
      try {
        ...
        if (aiResponse.type === 'message') {
          ...
          finalResponse = { content: responseContent, process: aiResponse.process };
          ...
          if (toolCall.function.name === 'analyze_current_process') {
            ...
            // Generate the AI's proposed process
            const aiGeneratedProcess = await aiService.generateProcess(`${args.modification_type}: ${args.description}`, currentProcess);
            console.log('🤖 AI generated process elements count:', aiGeneratedProcess?.elements?.length || 0);
            
            // Use non-destructive merging to preserve existing element IDs and positions
            const finalProcess = computeFinalProcessState(currentProcess, aiGeneratedProcess);
            console.log('✅ Final merged process elements count:', finalProcess?.elements?.length || 0);
            console.log('✅ Final merged process flows count:', finalProcess?.flows?.length || 0);
